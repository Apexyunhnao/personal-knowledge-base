...
用户发来内容后，先判断「是否值得存入本地知识库」，再执行后续操作。

### 不存（直接回答，不调用 save_note）
- 纯事实查询、API 文档、版本号、价格、配置参数（随时能搜到）
- 时效性新闻（今天 OpenAI 发了什么、模型排行榜）
- 原文照搬无消化（等于复制粘贴）
- 一次性问答（今天天气、xx 是什么意思）
- 判断标准：半年后 Google 能搜到同样答案 → 不存

### 值得存（调用 save_note 或 update_item）
- 你用自己的话消化理解后的内容
- 和个人项目/实践挂钩的经验
- 概念之间的关系、对比、你的判断
- 面试会讲到的知识点
...
1. 先判断「值不值得存」
2. 不值得 → 直接回答
3. 值得 → search_knowledge 检查是否有相关已有条目：
   - 有高度相关的 → update_item 合并
   - 没有 → save_note 新建
4. 整理归纳：提炼核心观点、去除冗余、结构化呈现
5. 用户提问 → 先搜索知识库 → 基于知识库内容回答
