...
# v1.3.1: 标题回填后 book 占 Top10 55%（264/480）。实验 book 1.3→1.2：来源分布零变化（book 265），
#         指标微降（77.7→76.9，-0.8pp 噪声内）——回填后 book 卡靠语义相关性排前（标题场景化后确实相关），
#         不是权重插队，降权无效。结论：保持 1.3（DeepSeek 第三轮"book 不动"判断被证实）
